...
def ddim_score_update2(score_model, sde, x_s, s, t, h=0.6, clamp = 10, device='cuda'):
    ...
    if s[0] < t_0:
        ...
        score_coeff = torch.pow(beta_step, 2/sde.alpha)*torch.pow(time_step, 1-2/sde.alpha)*np.sin(torch.pi/2*(2-sde.alpha))/(2-sde.alpha)*2/torch.pi*gamma_func(sde.alpha+1)
        noise_coeff = torch.pow(beta_step, 1 / sde.alpha)

        e_L = torch.clamp(levy.sample(sde.alpha, 0, size=x_s.shape).to(device), -clamp, clamp)

    else:
        beta_step = sde.beta(s-t_0) * time_step
        x_coeff = 1 + beta_step / 2
        noise_coeff = torch.pow(beta_step, 1 / 2)
        score_coeff = beta_step *2
        e_L = levy.sample(2, 0, size=x_s.shape).to(device)
...
def pc_sampler2(score_model,
    ...
    if datasets =="MNIST":
        if x_0:
            ...
            e_L = torch.clamp(e_L, -initial_clamp, initial_clamp)
        else:
         e_L = levy.sample(alpha, 0, (batch_size, 1, 28,28)).to(device)
        ...
        if x_0:
         ...
         x_s = levy.sample(2, 0, (batch_size, 3, 32, 32)).to(device)
    ...
    time_steps = torch.linspace(1, 1e-4, num_steps)
...
def ode_score_update(score_model, sde, x_s, s, t, clamp=3, h=0.001, return_noise=False):
    ...
    if s[0] < sde.t_0:
        ...
        beta_step = sde.beta(s-sde.t_0) * time_step
        x_coeff = 1 + beta_step / sde.alpha
        score_coeff = 1/2*sde.diffusion_coeff2(t-sde.t_0)*torch.pow(sde.diffusion_coeff2(s-sde.t_0), -1)*torch.pow(beta_step, 2/sde.alpha)*torch.pow(time_step, 1-2/sde.alpha)*np.sin(torch.pi/2*(2-sde.alpha))/(2-sde.alpha)*2/torch.pi*gamma_func(sde.alpha+1)
...
def dpm_score_update(model, sde, x_s, s, t, alpha, h=0.06, return_noise=False):
    ...
    score_s = model(x_s, s)*torch.pow( sde.beta(s) + 1e-5, -1)[:,None,None,None]
